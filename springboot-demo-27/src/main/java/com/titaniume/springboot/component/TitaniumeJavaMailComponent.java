...
import com.titaniume.springboot.util.configuration.TitaniumeJavaMailSenderImpl;
...
@Component
public class TitaniumeJavaMailComponent {
	private static final String template = "mail/titaniume.ftl";

	@Autowired
	private FreeMarkerConfigurer freeMarkerConfigurer;
	
	@Autowired
	private TitaniumeJavaMailSenderImpl javaMailSender;
	...
	private String send(String email, String text) throws MessagingException, UnsupportedEncodingException {
		...
		from.setPersonal("Titaniume", "UTF-8");
		helper.setFrom(from);
		helper.setTo(email);
